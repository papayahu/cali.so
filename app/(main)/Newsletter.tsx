...
export const newsletterFormSchema = z.object({
  email: z.string().email({ message: '信箱地址不正確' }).nonempty(),
  ...
  return (
    ...
    >
      ...
      <h2 className="flex items-center text-sm font-semibold text-zinc-900 dark:text-zinc-100">
        ...
        <span className="ml-2">動態更新</span>
      </h2>
      <p className="mt-2 text-xs text-zinc-600 dark:text-zinc-400 md:text-sm">
        <span>喜歡我的內容的話不妨訂閱支持一下 🫶</span>
        <br />
        {subCount && (
          <span>
            加入其他 <span className="font-medium">{subCount}</span> 位訂閱者，
          </span>
        )}
        <span>不定期發送，隨時可以取消訂閱。</span>
      </p>
